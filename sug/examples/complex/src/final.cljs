(ns examples.complex.final
  ...
  (:use
        [examples.complex.tokenize :only [tokenize-style remake register-node]]
...
(defn get-measure-style [node rule]
  (let [inline (aget (.-style node) rule)
        ...
        result (cond (= parsed "auto") 0;"auto"
                     (= parsed "") 0;""
                     (map? parsed) (cond
  ...
  (let []
    (dorun (map (fn [[token cache]]
           (let [el (:el token)
                 ...
                 [sbw sbh] (mapv - [sbr sbb] [sbx sby])
                 [nx ny] (mapv - (mapv * [x y] [sbw sbh]) ); (:off-in-box cache)  );(mapv - [sbx sby] [opx opy]))
                 [nw nh] (mapv int (mapv * [w h] [sbw sbh]))
                 [diff-x diff-y] (mapv int (mapv + [nx ny] [sx sy]))];(mapv + [sx sy] (mapv - [nx ny] [x y] [sx sy] [ml mt]))]

           (when (#{"" "static"} position)
            (aset (.-style el) "position" "relative"))
           (if (#{"" "static" "relative"} position)
             (.css (js/$ el) #js {:width (px nw) :height (px nh) })
...
;store the selection box bounds, for each element, store it's initial margins, dimensions, and float ratios for
;xywh and margins with respect to the selection box dimensions
;each update
...
;;              (aset (.-style el) "position" "relative"))
            (when-not resize-cache
              ...
              (def resize-cache
                (into {} (mapv (fn [n]
                                 (let [eloff (element-offset (:el n))
                                       lefttop [(get-measure-style (:el n) "left")
                                                (get-measure-style (:el n) "top")]
                                       [sx sy] lefttop
                                       xywh (xywh-ratio (:el n)  @SELECTION-BOX)
                                       [[sbx sby][sbr sbb]] @SELECTION-BOX
                                       [sbw sbh] (mapv - [sbr sbb] [sbx sby])
                                       [rat-x rat-y _ _] xywh]
                                   {n {:sx sx
                                       :sy sy
                                       :off-in-box (mapv * [rat-x rat-y] [sbw sbh])
...
(defn create-element [e [x y] data insertion -d -o]
  (let [app-state (get-in @data [:wrapper :app-state])
        ...
        j-ref-el (cond inside (js/$$ (get inside "element"))
                           (last after) (js/$$ (get (first after) "element"))
                           (first before) (js/$$ (get (last before) "element"))
                           :else (js/$$ "body"))
        ref-el (.get j-ref-el 0)
        offset-parent (.-offsetParent ref-el)
        [off-x off-y] (mapv - [x y] (element-offset offset-parent))
        offset (if (= "absolute" position)
                 (str "border:1px solid black;left:"  (px off-x) ";top:" (px off-y) ";")
                 "")
        new-el-str (str "<" el-type " style='position:" position ";width:1px;height:1px;" offset "'></" el-type ">")
        new-el (js/$ new-el-str)]
     (.log js/console new-el-str)
    (.log js/console (.first new-el))
    ;(.css new-el #js {:width (px 1) :height (px 1) :left (px off-x) :top (px off-y)})
    (cond (last after) (.before (js/$$ (get (first after) "element")) new-el)
          (first before) (.after (js/$$ (get (last before) "element")) new-el))

    (let [{:keys [dom nodes]} (register-node (.get new-el 0) (get-in @data [:wrapper :app-state :nodes]) (get-in @data [:wrapper :app-state :dom]))
          uid (aget (aget new-el 0) "uid")]
      ;(prn "[new]->" uid "  - " (uid nodes) )
      ;(.log js/console (.get new-el 0))
